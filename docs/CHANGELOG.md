...
否則就取消這個功能。這個判斷是對的 —— 全身圖縮到畫框角落後臉只有幾十像素、嘴巴只有幾像素，
在那個尺度上疊形狀或切下巴都只會像「嘴邊有東西在動」。

- **移除**：`Character.mouth`（嘴巴座標）、嘴巴位置推估、下巴分層與口腔暗部、介面上的「點圖標記嘴巴」。
- **改成**：`Character.mouth_shapes` —— 同一角色、不同嘴型的**整張**立繪 asset_id 清單；
  說話時逐格在這些圖之間輪替（含一格閉嘴，才有停頓感），並裁成胸上景讓差異看得出來。
  沒設就完全不做嘴型，立繪照原比例顯示（跟沒有這個功能時一樣）。
- 介面：角色卡的「嘴巴位置」換成「嘴型圖」多選；卡片狀態會直說「沒有嘴型圖 → 嘴巴不會動」。
- `estimate_portrait_geometry` 縮回 `estimate_head_box`（只量頭部盒，只在有嘴型圖時用來裁近景）。

---

## 動態漫畫：嘴型改成「胸上景 + 下巴開合」（2026-09-07）

回饋：嘴型看起來很不自然，像某個圖形在嘴邊動。原因是立繪是全身圖，縮到畫框角落後臉只有幾十像素、
嘴巴只有幾像素，那個尺度下疊任何形狀都只會像貼了一塊東西。

- **胸上景**：`estimate_portrait_geometry` 除了嘴巴也量頭部盒 (`head=[cx, top, bottom]`)，
  播放器據此把立繪裁成頭 + 肩胸（`Portrait.head`；量不到就整張顯示、不做嘴型）。臉約放大三倍。
- **下巴開合取代疊圖**：同一張原畫分 base（不動）與 jaw（只裁下顎橢圓、往下位移）兩層，
  兩層共用同一個 CSS 變數所以 base64 只嵌一份；口腔暗部夾在中間，下巴掉多少就露出多少。
  橢圓邊界落在臉與脖子內部，不會出現橫貫畫面的接縫。
- **暗部改成軟邊且只在張大時浮出**（`open > 0.32`），並比估到的嘴巴窄、往下偏半個嘴高：
  硬邊實心的版本在嘴線估得偏高時會變成「鬍子」。
- **嘴線預設由頭高 72% 改 78%**：拿七張角色設定稿比對，72% 會落在鼻子；78% 對寫實與動漫畫風都
  落在唇上。寧可偏低（只是下巴多動一點）也不要偏高（很明顯是錯的）。
- 測試改以下巴 transform 與下顎橢圓裁切驗證，並確認立繪 base64 只嵌一次。

---

## 漫畫站：角色演出編輯介面（2026-09-06）

上一版的表情 / 嘴型只能走 API 設定，這版補上介面。

- **Assets 分頁新增「角色演出（表情 · 嘴型）」面板**：每個角色一張卡 —— 選立繪
  (`character_anchor`)、點立繪標嘴巴位置（可調寬高、可清除改回自動推估）、逐表情挑變體 asset，
  卡上直接顯示「會演出 / 不演出」與原因。改完按「儲存角色演出」寫回 Series。
- **頁面編輯 → 泡泡版面與指向設定**：每句對白多一個表情下拉（預設「依語氣自動」）。
- **修掉既有 bug**：存 Series Bible 會把角色的 `anchor_assets` 洗成空的（文字框重建角色時沒有保留）。
  現在依 `character_id` 合併，`anchor_assets` / `expressions` / `mouth` 都保住 —— 否則設完演出再存一次
  Bible 就全沒了。
- 純邏輯抽到 `frontend/edustudio/comic-cast.js`（`node --test` 18 項），JSX 只負責畫面。
- 實機驗證：起 server + 真的開 `/app` 漫畫站，走完選立繪 → 點圖標嘴 → 設表情變體 → 儲存 → 重讀確認落地，
  以及逐句表情選單存檔。
