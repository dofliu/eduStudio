...
11. **角色演出(表情 · 嘴型)**:角色只要有 `character_anchor` 立繪,說話時就會在畫框角落現身
    (自動站在泡泡的另一側),並且:
    - **表情**:依台詞語氣自動判斷(驚訝 / 疑問 / 生氣 / 擔心 / 開心 / 沉吟),換成對應的表情立繪
      並帶情緒符號與動態。想指定就在對白填 `expression`,想準備表情變體就在角色填
      `expressions`(表情名 → asset_id);沒有變體也會有動態與符號,不會沒反應。
    - **嘴型**:講話時嘴會逐格開合(閉嘴時完全透明,看到的是原畫的嘴)。嘴巴位置從**去背立繪**的
      輪廓自動推估(頭頂 → 肩膀 → 嘴在頭高七成處);位置不準時在角色填 `mouth`
      (`[cx, cy, w, h]`,相對立繪的 0~1)手動修正。立繪**要去背**(PNG 透明底)才推得出來。
