...
- **講者備註 → 旁白**:PPTX 按「轉影片」時,會讀原簡報每頁的**備忘稿**當該頁旁白的**權威大綱** —
  涵蓋你寫的每個重點、照你的順序、展開成口語;與畫面有出入時以備註為準。沒寫備註的頁維持原本
  純看圖生成。備註越具體,旁白越像你自己講的。(頁數對不上時整份不使用,不會錯位套到別頁。)
...
11. **角色演出(表情 · 嘴型)**:在 **Assets 分頁 → 「角色演出(表情 · 嘴型)」** 面板設定 —
    每個角色選一張 `character_anchor` 當立繪(旁白選的則會當片頭 / 片尾卡與字幕條旁的頭像)。
    設好之後,角色說話時就會在畫框角落現身(自動站在泡泡的另一側),並且:
    - **表情**:依台詞語氣自動判斷(驚訝 / 疑問 / 生氣 / 擔心 / 開心 / 沉吟),換成對應的表情立繪
      並帶情緒符號與動態。要為某個表情準備專屬立繪,在該角色卡的「表情變體」挑對應的
      `character_anchor`;沒準備變體的表情會用預設立繪,改以動態與符號表現,不會沒反應。
      想指定某一句的表情,到**頁面編輯 → 泡泡版面與指向設定**,那一句選「表情:○○」
      (預設「依語氣自動」)。
    - **鏡位**:說話時自動裁成**胸上景**(頭 + 肩胸)。全身圖縮到畫框角落時臉只有幾十像素,
      嘴巴更只有幾像素,那個尺度下再怎麼做嘴型都只會像「嘴邊有東西在動」。
    - **嘴型**:講話時**下巴會跟著開合** —— 沿嘴線把原畫切開,下半往下掉,縫隙露出口腔的暗部;
      動的是畫本身,不是在臉上疊形狀。嘴巴位置預設從**去背立繪**的輪廓自動推估
      (頭頂 → 肩膀 → 嘴在頭高 78% 處);推得不準就在角色卡按「點圖標記」,直接點立繪上的嘴巴,
      再用「寬 / 高 %」調大小,按「清除」可改回自動。立繪**要去背**(PNG 透明底)才推得出來。
    - 每張角色卡右上角會顯示**會演出 / 不演出**與原因(例:沒指定立繪、指到這一集沒有的 asset),
      改完按**「儲存角色演出」**存回 Series(整個連載共用)。
12. **手繪風格轉場**:換頁不再只是淡接,而是用鋸齒邊把新頁「刷」出來並沿邊壓一條墨線,進度量化成
    逐格 — 墨刷(預設,隔頁交替方向)、撕紙(特寫頁)、速度線(動作橋段,依 camera / beat 判斷)。
